package com.jd.binlog.server.parser;

import com.google.protobuf.ByteString;
import com.jd.binlog.dbsync.LogEvent;
import com.jd.binlog.dbsync.event.DeleteRowsLogEvent;
import com.jd.binlog.dbsync.event.IntvarLogEvent;
import com.jd.binlog.dbsync.event.LogHeader;
import com.jd.binlog.dbsync.event.QueryLogEvent;
import com.jd.binlog.dbsync.event.RandLogEvent;
...
import com.jd.binlog.dbsync.event.RowsQueryLogEvent;
import com.jd.binlog.dbsync.event.TableMapLogEvent;
import com.jd.binlog.dbsync.event.UnknownLogEvent;
import com.jd.binlog.dbsync.event.UpdateRowsLogEvent;
import com.jd.binlog.dbsync.event.UserVarLogEvent;
import com.jd.binlog.dbsync.event.WriteRowsLogEvent;
import com.jd.binlog.dbsync.event.XidLogEvent;
import com.jd.binlog.server.common.AviaterRegexFilter;
import com.jd.binlog.server.common.BinlogPipelineException;
import com.jd.binlog.server.common.ErrorCode;
import com.jd.binlog.server.common.TableMeta;
...
public class LogEventConvert {
    ...
    public static final Logger logger              = Logger.getLogger(LogEventConvert.class);
    ...
    public CanalEntry.Entry parse(LogEvent logEvent) throws BinlogPipelineException {
        ...
        if (StringUtils.endsWithIgnoreCase(queryString, BEGIN)) {
            ...
            if (result.getType() == CanalEntry.EventType.ALTER || result.getType() == CanalEntry.EventType.ERASE
                ...
                if (StringUtils.isEmpty(tableName)
                    ...
                    throw new BinlogPipelineException(ErrorCode.PARSER_ERROR,
    ...
    private CanalEntry.Entry parseRowsQueryEvent(RowsQueryLogEvent event) {
        ...
        try {
            ...
            throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, e);
        }
    }
    ...
    private CanalEntry.Entry parseRowsEvent(RowsLogEvent event) {
        try {
            ...
            if (table == null) {
                // tableId对应的记录不存在
                throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, "not found tableId:" + event.getTableId());
            }
            ...
            if (LogEvent.WRITE_ROWS_EVENT_V1 == type || LogEvent.WRITE_ROWS_EVENT == type) {
                ...
                throw new BinlogPipelineException(ErrorCode.NOT_SUPPORT_EVENT, "event type :" + event.getHeader().getType());
            }
            ...
            if (tableMetaCache != null) {// 入错存在table meta cache
                ...
                if (tableMeta == null) {
                    throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, "not found [" + fullname + "] in db , pls check!");
                }
            }
            ...
            throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, "parse row data failed.", e);
        }
    }
    ...
    private void parseOneRow(CanalEntry.RowData.Builder rowDataBuilder, RowsLogEvent event, RowsLogBuffer buffer, BitSet cols,
        ...
        if (tableMeta != null && columnInfo.length > tableMeta.getFileds().size()) {
            ...
            tableMeta = tableMetaCache.getTableMeta(event.getTable().getDbName(),
                    event.getTable().getTableName());// 强制重新获取一次
            if (tableMeta == null) {
                throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, "not found ["
            ...
            // 在做一次判断
            if (tableMeta != null && columnInfo.length > tableMeta.getFileds().size()) {
                throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, "column size is not match for table:"
    ...
    private boolean isUpdate(List<CanalEntry.Column> bfColumns, String newValue, int index) {
        if (bfColumns == null) {
            throw new BinlogPipelineException(ErrorCode.PARSER_ERROR, "the bfColumns is null");
        }
