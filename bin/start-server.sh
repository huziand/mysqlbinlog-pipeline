...
if [ -f $base/bin/BinlogPipelineServer.pid ] ; then
...
SERVER_OPTS="-DappName=binlog-pipeline -Dconfig=$conf"
...
then
    ...
    cd $bin_abs_path
    cd $base
    $JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $SERVER_OPTS -classpath .:$CLASSPATH com.jd.binlog.server.BinlogPipelineServer 1>>$base/logs/binlog-pipeline.log 2>&1 &
    echo $! > $base/bin/BinlogPipelineServer.pid

    echo "BinlogPipelineServer started"
    cd $current_path
else
